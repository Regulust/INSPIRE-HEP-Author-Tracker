...
const PROJECT_LINK = "https://github.com/your-github-username/inspire-hep-author-tracker";
...
function renderReport({ authors, days, startDate, endDate, queryTime, results, abortError }) {
  ...
  const authorSections = results
    .filter(Boolean)
    .map(({ author, papers, error, warning }, authorIndex) => {
      ...
      return `
        <section>
          <h2>${sanitizeText(author.name)}</h2>
          <p><a href="${sanitizeText(author.url)}" target="_blank" rel="noreferrer">Author profile</a></p>
          ${paperHtml}
        </section>
        ${authorIndex < results.filter(Boolean).length - 1 ? "<hr />" : ""}
      `;
    })
    .join("");
